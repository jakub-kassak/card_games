...
class StateVariables(PClass):
    ace: int = field(type=int, mandatory=True, invariant=lambda x: (x >= 0, 'ace counter can not be negative'), initial=0)
    suit: Suit = field(type=Suit, mandatory=True, initial=Suit.BELL)
    val: Value = field(type=Value, mandatory=True, initial=Value.X)
    cnt: int = field(type=int, mandatory=True, invariant=lambda x: (x >= 0, 'draw count must be positive'), initial=1)
